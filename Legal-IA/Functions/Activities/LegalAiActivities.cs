    ...
    IInvoiceItemRepository invoiceItemRepository)
{
    ...
    public async Task<UserFullContext?> BuildUserFullContextActivity([ActivityTrigger] BuildUserFullContextInput input)
    {
        try
        {
            ...
            if (input.IncludeUserData)
            {
                ...
                var currentQuarterInvoices = currentYearInvoices
                    .Where(i => GetQuarterFromDate(i.IssueDate) == currentQuarter).ToList();
    ...
    public async Task<LegalQueryResponse> ProcessLegalQuestionWithFullContextActivity(
        [ActivityTrigger] ProcessLegalQuestionInput input)
    {
        try
        {
            ...
            if (input.UserFullContext != null)
                return await legalAiAgent.ProcessQuestionWithFullContextAsync(input.Request, input.UserFullContext,
    ...
    public async Task<string> GetFormGuidanceWithFullContextActivity([ActivityTrigger] FormGuidanceInput input)
    {
        try
        {
            ...
            if (input.UserFullContext != null)
                return await legalAiAgent.GetFormGuidanceWithFullContextAsync(input.Question, input.FormType,
                    input.UserFullContext, CancellationToken.None);
    ...
    public async Task<string> GetQuarterlyObligationsWithFullContextActivity(
        [ActivityTrigger] QuarterlyObligationsInput input)
    {
        try
        {
            logger.LogInformation("Getting quarterly obligations with full context for Q{Quarter} {Year}",
                input.Quarter, input.Year);

            if (input.UserFullContext != null)
                return await legalAiAgent.GetQuarterlyObligationsWithFullContextAsync(input.Question, input.Quarter,
                    input.Year, input.UserFullContext, CancellationToken.None);

            return await legalAiAgent.GetQuarterlyObligationsAsync(input.Quarter, input.Year);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting quarterly obligations with full context for Q{Quarter} {Year}",
                input.Quarter, input.Year);
    ...
    public async Task<string> GetAnnualObligationsWithFullContextActivity(
        [ActivityTrigger] AnnualObligationsInput input)
    {
        try
        {
            ...
            if (input.UserFullContext != null)
                return await legalAiAgent.GetAnnualObligationsWithFullContextAsync(input.Question, input.Year,
                    input.UserFullContext, CancellationToken.None);
