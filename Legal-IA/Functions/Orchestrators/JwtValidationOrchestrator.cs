...
public class JwtValidationOrchestrator
{
    ...
    public async Task<JwtValidationResult> Run([OrchestrationTrigger] TaskOrchestrationContext context)
    {
        ...
        _logger.LogInformation("Starting JWT validation orchestration. Token received: {Token}",
        ...
        try
        {
            ...
            if (!result.IsValid)
                _logger.LogWarning("JWT validation failed. Token: {Token}",
                    jwt != null ? jwt.Substring(0, Math.Min(jwt.Length, 10)) + "..." : "null");
            else
                _logger.LogInformation("JWT validation succeeded. Claims: {Claims}",
                    result.Claims != null
        ...
        {
            _logger.LogError(ex, "Exception during JWT validation orchestration. Token: {Token}",
                jwt != null ? jwt.Substring(0, Math.Min(jwt.Length, 10)) + "..." : "null");
