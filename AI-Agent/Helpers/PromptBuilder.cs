...
{
    /// <summary>
    ///     Builds a generic system prompt for the AI agent, instructing it to answer only legal, tax, or judicial questions
    ...
    public static string BuildSystemPrompt(UserFullContext userFullContext)
    {
        ...
        sb.AppendLine(
            "Eres un asistente experto en derecho y fiscalidad para autónomos en España. Responde de manera clara, precisa y profesional.\nSi la pregunta no es legal o fiscal, indica que solo puedes responder sobre temas legales, fiscales o judiciales en España.");
        sb.AppendLine(
        ...
        sb.AppendLine(
            $"Total facturas este trimestre: {invoices.Count(i => i.IssueDate.Year == DateTime.Now.Year && (i.IssueDate.Month - 1) / 3 + 1 == (DateTime.Now.Month - 1) / 3 + 1)}");
        sb.AppendLine(
            $"Ingresos año actual: {user.TotalIncomeCurrentYear}, IVA año actual: {user.TotalVATCurrentYear}, IRPF año actual: {user.TotalIRPFCurrentYear}");
        sb.AppendLine();
        ...
        if (invoices.Count > 0)
            foreach (var i in invoices.OrderByDescending(i => i.IssueDate).Take(10))
                sb.AppendLine(
                    $"- Nº: {i.InvoiceNumber}, Fecha: {i.IssueDate:yyyy-MM-dd}, Cliente: {i.ClientName}, Total: {i.Total:C}, IVA: {i.VAT:C}, IRPF: {i.IRPF:C}");
        else
...
";
    ...
    public static string BuildPersonalizedFormPrompt(AutonomoFormRequest request, UserFullContext userFullContext)
    {
        ...
        var invoices = userFullContext.Invoices;
        var invoiceSummary =
            $"Total facturas este año: {invoices.Count}, Total facturas este trimestre: {invoices.Count(i => i.IssueDate.Year == DateTime.Now.Year && (i.IssueDate.Month - 1) / 3 + 1 == (DateTime.Now.Month - 1) / 3 + 1)}";
        var incomeSummary =
            $"Ingresos año actual: {user.TotalIncomeCurrentYear}, IVA año actual: {user.TotalVATCurrentYear}, IRPF año actual: {user.TotalIRPFCurrentYear}";
...
Responde de manera profesional y práctica.";
    ...
    /// <param name="userFullContext">The full user context including invoices and summary data.</param>
    /// <returns>A structured prompt string for the AI agent.</returns>
    public static string BuildFormPromptWithFullContext(string question, string formType,
        UserFullContext userFullContext)
    {
        ...
        sb.AppendLine("--- INSTRUCCIONES IMPORTANTES Y ESTRICTAMENTE NECESARIAS ---");
        sb.AppendLine(
            $"Como experto en fiscalidad para autónomos en España, responde la pregunta específica de {user.FirstName} {user.LastName} sobre el {formType}.");
        sb.AppendLine(
            "Responde de manera clara, precisa y profesional, como lo haría un asesor fiscal humano. No incluyas advertencias, disclaimers ni referencias a ser un modelo de lenguaje. Limítate a proporcionar la información solicitada.");
        sb.AppendLine();
        ...
        sb.AppendLine(
            "Usa esta información para proporcionar una respuesta personalizada y específica para su situación.");
    ...
    /// <param name="userFullContext">The full user context including invoices and summary data.</param>
    /// <returns>A structured prompt string for the AI agent.</returns>
    public static string BuildQuarterlyObligationsPromptWithFullContext(string question, int quarter, int year,
        UserFullContext userFullContext)
    {
        var user = userFullContext.UserContext;
        var invoices = userFullContext.Invoices;
        ...
        sb.AppendLine("--- INSTRUCCIONES IMPORTANTES Y ESTRICTAMENTE NECESARIAS ---");
        sb.AppendLine(
            $"Como experto fiscal para autónomos en España, responde la pregunta específica de {user.FirstName} {user.LastName} sobre las obligaciones del {quarter}º trimestre de {year}.");
        sb.AppendLine(
            "Responde de manera clara, precisa y profesional, como lo haría un asesor fiscal humano. No incluyas advertencias, disclaimers ni referencias a ser un modelo de lenguaje. Limítate a proporcionar la información solicitada.");
        sb.AppendLine();
        ...
        sb.AppendLine(
            "Usa esta información para proporcionar una respuesta personalizada considerando su situación fiscal específica.");
    ...
    /// <param name="userFullContext">The full user context including invoices and summary data.</param>
    /// <returns>A structured prompt string for the AI agent.</returns>
    public static string BuildAnnualObligationsPromptWithFullContext(string question, int year,
        UserFullContext userFullContext)
    {
        var user = userFullContext.UserContext;
        var invoices = userFullContext.Invoices;
        ...
        sb.AppendLine("--- INSTRUCCIONES IMPORTANTES Y ESTRICTAMENTE NECESARIAS ---");
        sb.AppendLine(
            $"Como experto fiscal para autónomos en España, responde la pregunta específica de {user.FirstName} {user.LastName} sobre las obligaciones anuales del año {year}.");
        sb.AppendLine(
            "Responde de manera clara, precisa y profesional, como lo haría un asesor fiscal humano. No incluyas advertencias, disclaimers ni referencias a ser un modelo de lenguaje. Limítate a proporcionar la información solicitada.");
        sb.AppendLine();
        ...
        sb.AppendLine(
            "Usa esta información para proporcionar una respuesta personalizada considerando su situación fiscal específica del año completo.");
